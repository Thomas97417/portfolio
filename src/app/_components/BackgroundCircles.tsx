...
export const BackgroundCircles = ({ parallaxRef }: BackgroundCirclesProps) => {
  ...
  return (
    <div className="absolute -top-[2vh] left-1/2 h-screen w-[90vw] -translate-x-1/2 max-lg:hidden ">
      {/* Moving background colored circle balls */}
      <MouseParallax strength={0.07} parallaxContainerRef={parallaxRef}>
        <div className="w-0.25 absolute bottom-1/2 left-1/2 h-1/2 origin-bottom rotate-[46deg]">
          <div
            className={`-mt-30 -ml-1 h-2 w-2 rounded-full bg-gradient-to-b from-[#DD734F] to-[#1A1A32] transition-transform duration-500 ease-out ${
        ...
        <div className="w-0.25 absolute bottom-1/2 left-1/2 h-1/2 origin-bottom rotate-[54deg]">
          <div
            className={`-ml-1 mt-[12.9rem] h-4 w-4 rounded-full bg-gradient-to-b from-[#B9AEDF] to-[#1A1A32] transition-transform duration-500 ease-out ${
              mounted ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"
            }`}
          />
        </div>
